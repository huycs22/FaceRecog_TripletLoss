...
class MapFunction():
	...
	def augment(self, image):
        ...
        # 2) Random small rotation via Keras layer
		image = self.random_rot(image)
        # 3) Random brightness: delta in [-0.2, +0.2]
	...
	def __call__(self, anchor, positive, negative):
		...
		return (self.augment(anchor), self.augment(positive), self.augment(negative))
...
class TripletGenerator:
	...
	def get(self):
		...
		# yield the anchor, positive and negative photos
		return (anchorPhoto, positivePhoto, negativePhoto)
	
	def getDataSet(self, samples):
		anchors = []
		positives = []
		negatives = []
		for i in range(samples):
			example = self.get()
			anchors.append(example[0])
			positives.append(example[1])
			negatives.append(example[2])
